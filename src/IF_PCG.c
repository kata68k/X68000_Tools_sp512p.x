...
void PCG_INIT(int16_t num)
{
	...
	puts("SP/BG mode");
	#if XEiJ_Ex_SP
//	*XEiJ_ExSP_BNKCTRL = Mbset(*XEiJ_ExSP_BNKCTRL, (Bit_2|Bit_1|Bit_0), (Bit_2|Bit_0));	/* バンク制御レジスタ */
//	*XEiJ_ExSP_BNKSEL = Mbset(*XEiJ_ExSP_BNKSEL, (Bit_11|Bit_10|Bit_9|Bit_8), 0);	/* バンク選択レジスタ */
	#endif
#endif  /*CNF_XSP*/
	...
	if(ret < 0)
	{
		printf("PCG_INIT SP_INIT_Ex() error\n", ret);
		return;
	}
	/*スプライトレジスタ初期化*/
	for( j = 0x80000000; j < 0x80000000 + PCG_CODE_MAX; j++ )
	{
		ret = _iocs_sp_regst(j,-1,0,0,0,0);
#ifdef DEBUG	/* デバッグコーナー */
		if(ret < 0)
		{
			printf("PCG_INIT _iocs_sp_regst() error\n", ret);
		}
#endif
	}
	for( i = 0; i < PCG_CODE_MAX; i++ )
	{
		ret = _iocs_sp_cgclr(i);			/* スプライトクリア */
#ifdef DEBUG	/* デバッグコーナー */
		if(ret < 0)
		{
			printf("PCG_INIT _iocs_sp_regst() error\n", ret);
...
void PCG_BG_INIT(void)
{
	...
	/* BGの設定用（XSP以外はSPの設定） */
	for( i = 0; i < PCG_8x8_AREA; i++ )
	{
		_iocs_sp_defcg( i, SP_8x8, &g_pcg_dat[i * SP_8_SIZE] );
	}
	for( i = 0; i < PCG_16x16_AREA; i++ )
	{
		_iocs_sp_defcg( i, SP_16x16, &g_pcg_dat[i * SP_16_SIZE] );
	}
}
...
void PCG_ON(void)
{
	...
	for (i = 0; i < 256; i++) {
		pcg_roll90(&g_pcg_dat[i * SP_16_SIZE], 1);
	}
#endif
...
#else   /* CNF_XSP 0 */
		/*スプライトレジスタ初期化*/
		for( i = 0x80000000; i < 0x80000000 + PCG_CODE_MAX; i++ )
		{
			_iocs_sp_regst(i,-1,0,0,0,0);
		}
#endif  /*CNF_XSP*/
	}
}
