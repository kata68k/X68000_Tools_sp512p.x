...
/* 機能		：	*/
/*===========================================================================================*/
int16_t CRTC_Image_Get(void)
{
	int16_t	ret = 0;
	static int8_t s_bFlag = FALSE;
	static int8_t s_cCount = 0;

	if((*CRTC_480 & Bit_0) != 0u)
	{
//		printf("CRT()=0x%x 出力モード\n", *CRTC_480);
//		KeyHitESC();	/* デバッグ用 */
//		*CRTC_480 = Mbclr(*CRTC_480, Bit_0);		/* 出力モード */
//		printf("CRT()=0x%x 出力モード\n", *CRTC_480);
//		KeyHitESC();	/* デバッグ用 */
	}
	else
	{
		...
		{
			s_bFlag = TRUE;
			printf("CRT()=0x%x 入力モード(変更前)\n", *CRTC_480);
	//		KeyHitESC();	/* デバッグ用 */
	//		*CRTC_R21 = Mbset(*CRTC_R21, 0x03FF, 0x0F);	/* CRTC R21 */
			*CRTC_480 = Mbset(*CRTC_480, Bit_0, 1);	/* 入力モード */
	//		*CRTC_480 = 1;	/* 入力モード */
			printf("CRT()=0x%x 入力モード(変更後)\n", *CRTC_480);
	//		KeyHitESC();	/* デバッグ用 */
		}
	}
	printf("CRT()=0x%x 監視(%03d)\n", *CRTC_480, s_cCount++);
