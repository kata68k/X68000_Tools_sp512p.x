...
int16_t GetFileLength(int8_t *, size_t *);
...
/* size		データのサイズ */
/* n		データの個数 */
size_t File_Load_Split(int8_t *fname, void *ptr, size_t buffer_size, size_t size, size_t n, int32_t num)
{
	FILE *fp;
	size_t ret = 0;
	...
	if (read_size > buffer_size) {
		fprintf(stderr, "Buffer overflow risk! (%zu > %zu)\n", read_size, buffer_size);
		return 0;
	}
	
	if(ptr == NULL){
//		fprintf(stderr, "Buffer NULL! (0x%p)\n", ptr);
		return 0;
	}
...
int16_t PCG_SP_dataload(int8_t *fname)
{
	...
	pcg_dat = (uint8_t *)MyMalloc(PCG_CODE_MAX * SP_16_SIZE);
	...
	else
	{
		...
		pcg_size = Mmin(pcg_size, PCG_CODE_MAX * SP_16_SIZE);	/* BGの設定状態でサイズが異なる？？ */
...
int16_t PCG_PAL_dataload(int8_t *fname)
{
	...
	else
	{
		...
		for( i = 0 ; i < 256 ; i++ )
		{
			_iocs_spalet( (0x80000000 | (i & 0x0F)), Mdiv16(i) + 1 , pal_dat[i] ) ;	/* テキスト共有エリアを除く */
		}
	}
			...
			else
			{
				...
				else
				{
					if(i < num)
					{
						for(j=i; j < num; j++)
				{
							sprintf(list[j], "%s%s", fpath, "null");
							i++;
						}
						j=0;
				}
					sprintf(list[num], "%s%s", fpath, z_name);
				i++;
			}
			}
			memset(buf, 0, sizeof(buf));
	...
	else
	{
		...
		while(fgets(buf, sizeof(buf), fp) != NULL)
		{
			...
			else
			{
				...
				nKey = 0;
				memset(z_name, 0, sizeof(z_name));
				ret = sscanf(p,"%d= %s %d %d %d", &num, z_name, &nGR, &nSP, &nKey);	/* format:番号 = ファイルパス＆ファイル名,グラフィック,スプライト */
				if(i == num)
				{
...
/* 機能		：	*/
/*===========================================================================================*/
int16_t	GetFileLength(int8_t *pFname, size_t *pSize)
{
	FILE *fp;
	int16_t ret = 0;
	size_t	Tmp;
