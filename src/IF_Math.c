...
// 並び替え用関数
// input: 並び替える元の16ビット値
// order: 並び順の配列（サイズ16） 例: {15, 14, ..., 0} でビット反転
uint16_t reorder_bits(uint16_t input, const uint8_t order[16]) 
{
    uint16_t output = 0;
	int32_t i;

    for (i = 0; i < 16; ++i) {
        // 元のinputのorder[i]番目のビットを取り出す
        uint16_t bit = (input >> order[i]) & 1;
        // i番目の位置に配置
        output |= (bit << i);
    }
...
/* 機能		：	*/
/*===========================================================================================*/
// 上位8ビットを右に8ビットシフト、下位8ビットを左に8ビットシフトして合成
uint16_t swap_nibbles8(uint16_t word)
{
...
/* 機能		：	*/
/*===========================================================================================*/
// 上位4ビットを右に4ビットシフト、下位4ビットを左に4ビットシフトして合成
