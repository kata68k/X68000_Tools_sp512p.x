...
static volatile uint8_t 	g_bTimer_D_subcnt = 0;
static volatile uint8_t 	g_bTimer_D_subcntMax = 3;
...
uint16_t TimerD_INIT(uint16_t, uint16_t, uint16_t, uint16_t);
...
int16_t MFP_INIT(void)
{
	...
	g_CPU_Time = TimerD_INIT(223, 6617, 10000, 3);		/* Timer-D初期化 */
...
/* 機能		：	*/
/*===========================================================================================*/
uint16_t	TimerD_INIT(uint16_t base, uint16_t add, uint16_t range, uint16_t cnt)
{
	uint16_t CpuCount=0;
	int32_t nUse;
	/* Timer-D割り込み */
	...
	g_stTimer_D.base = base;
	g_stTimer_D.add = add;
	g_stTimer_D.range = range;
	g_stTimer_D.err = 0;

	g_bTimer_D_subcnt = 0;
	g_bTimer_D_subcntMax = cnt;

	//	nUse = _iocs_timerdst(Timer_D_Func, 7, 20);	/* Timer-Dセット */	/* 50us(7) x 20cnt = 1ms */
	nUse = _iocs_timerdst(Timer_D_Func, 7, base);	/* Timer-Dセット */	/* 50us(7) x 679.88cnt = 33.39942ms 33.4ms /3 = 169.97 */
...
/* 機能		：	*/
/*===========================================================================================*/
uint16_t frac_timer_next(frac_timer_t *t)
{
    uint16_t val = t->base;
...
static void interrupt Timer_D_Func(void)
{
    ...
    set = g_stTimer_D.base;

    g_stTimer_D.err += g_stTimer_D.add;

    if (g_stTimer_D.err >= g_stTimer_D.range) {
        g_stTimer_D.err -= g_stTimer_D.range;
        set -= 1;
    }

	_iocs_timerdst(Timer_D_Func, 7, set);	/* Timer-Dセット */	/* 50us(7) x 679.88cnt = 33.39942ms 33.4ms /3 = 169.97 */

	g_bTimer_D_subcnt++;

	if (g_bTimer_D_subcnt >= g_bTimer_D_subcntMax) {
		g_bTimer_D_subcnt = 0;
